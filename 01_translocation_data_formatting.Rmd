...
```{r}
lat.df<-read_xlsx("C:/Users/Steph/TAMU_OneDrive/Thrushes/translocation/data_archiving/translocation_data_revision.xlsx",sheet=5)
...
ggplot(birds,aes(x=hiest_ancestry,y=hiest_heterozygosity,colour=as.character(survival)))+
...
ch.df<-ch.df%>%filter(name_in_vcf%in%birds$name_in_vcf)%>%
  ...
  select(name_in_vcf,motustagid,hiest_ancestry,release_site,sex_binary,ch)
...
ch2<-ch2%>%left_join(birds)%>%
  mutate(ch=paste("days",ch,sep="_"))%>% #stops capture history from being saved as a very large number
  select(name_in_vcf,motustagid,hiest_ancestry,release_site,release_year,sex_binary,ch)
# write.csv(ch2,
...
#set release site factors and flip ancestry estimates so inland=1
lat.df.birds$release_site<-factor(lat.df.birds$release_site,levels=c("Squamish","Pemberton","Lilloet"))
lat.df.birds<-lat.df.birds%>%left_join(meta_thrush%>%dplyr::select(name_in_vcf,hiest_ancestry))
lat.df.birds$ancestry<-lat.df.birds$hiest_ancestry*(-1)+1

#get unique detections
lat.df.birds<-lat.df.birds%>%
  select(recvDeployLon,recvDeployLat,name_in_vcf,ancestry,release_site,
         deployDay,deploydoy,detectdoy,detectyear,detectmonth,tagDays)%>%
  distinct()

map1<-ggplot() +
  geom_sf(data = world,fill="grey90",colour=NA)+
  geom_path(data=lat.df.birds,aes(x=recvDeployLon,y=recvDeployLat,
                                  group=name_in_vcf,colour=ancestry),size=0.1,alpha=0.4)+
  
  geom_point(data=lat.df.birds,aes(x=recvDeployLon,y=recvDeployLat,
                                colour=ancestry,group=name_in_vcf),size=1)+
  
  scale_colour_scico(palette="navia",name="Ancestry",end=0.9,limits=c(0,1))+
  
  xlab("Longitude")+ylab("Latitude")+
  ylim(5,55)+xlim(-130,-70)+
  facet_wrap(vars(release_site))

map1


ggsave("C:/Users/Steph/TAMU_OneDrive/Thrushes/translocation/Fig4.svg",
       plot=map1,width=10,height=3)
...
library(wesanderson)
gg3c<-ggplot() +
  geom_sf(data = world,fill="grey99",colour="grey80")+
...
ggsave("C:/Users/Steph/TAMU_OneDrive/Thrushes/translocation/Fig3C.svg",
       plot=gg3c,width=8,height=3)
