...
ggplot(birds,aes(x=ancestry,y=norm_releaseSite_bearing_1,
  ...
  scale_color_manual(values=scico(n=3,palette='navia',end=0.8)[c(3,1)],name="Release site")+
  xlab("Ancestry")+
  ylab("Orientation to\nrelease site")
...
gg1<-ggplot(p$fit, aes(release_site, visregFit,
  ...
  scale_color_manual(values=scico(n=3,palette='navia',end=0.8,direction=-1),name="migratory\ngenotype")+
  labs(fill="Weight")+
...
p1<-ggplot(birds,aes(x=ancestry,y=norm_releaseSite_bearing_1,
                 group=release_site,
                 colour=release_site,fill=release_site))+
  geom_point(size=2,shape=19,alpha=0.7)+
  geom_smooth(method="lm",alpha=0.1)+
  scale_color_manual(values=scico(n=3,palette='navia',end=0.8,direction=-1),name="Release site")+
  scale_fill_manual(values=scico(n=3,palette='navia',end=0.8,direction=-1),name="Release site")+
  xlab("Ancestry")+
  ylab("Orientation")
...
lm2b<-lm(norm_releaseSite_bearing_1~f_release_year+f_release_site*ancestry,
            data=birds2,na.action=na.exclude)
Anova(lm2b)
...
lm2c<-lm(norm_releaseSite_bearing_1~f_release_year*ancestry,
            data=birds2%>%filter(release_site=="Pemberton"),
         na.action=na.exclude)
Anova(lm2c)
...
gg1<-ggplot(p$fit, aes(f_release_site,
  ...
  scale_color_manual(values=scico(n=3,palette='navia',end=0.8),name="Ancestry",
                     labels=c("Coastal backcross","Hybrid","Inland backcross"))+
  labs(fill="Weight")+
  ylab("Orientation")+xlab("Release site")
gg1
#ggsave(plot=gg1,
#       file="C:/Users/Steph/TAMU_OneDrive/Thrushes/translocation/Fig3B.svg",
#       width=5.5,height=4)
